    ...
    internal static class Formatter
    {
        public static string Format(DeadLine ddl)
        {
            ...
            if (remain.TotalHours >= 24)
                return $"{ddl.DueDate:yyyyMMdd} +++ {remain.Hours,9} hours {ddl.Name}/d/{ddl.Title}";
            if (remain.TotalHours >= 2)
                return $"{ddl.DueDate:yyyyMMdd} ++++ {remain.Hours,8} hours {ddl.Name}/d/{ddl.Title}";
            if (remain.Minutes >= 2)
                return $"{ddl.DueDate:yyyyMMdd} +++++++ {remain.Minutes,5} mins  {ddl.Name}/d/{ddl.Title}";
            return $"{ddl.DueDate:yyyyMMdd} ++++++++ {remain.Seconds,4} secs  {ddl.Name}/d/{ddl.Title}";
        }
    }
}
